...
## The bed of the shelf

The shelf keeps deepening past the knee of the near-shore curve: `terrainShelfSlope` (m per 100 m, from 24 m offshore to the strip edge at 96 m) is part of the geometry key, mirrored in `coastShelfDrop` on both sides, so the sand fades into the water body before the strips end instead of stopping on a drawn line. The shore strip hands over to the far body across 96–64 m and carries the far field's swell (`farWaterSwellShader`, shared with `FarWaterSurface`) over its outer fifty metres; the far field's cut-out stops half a metre short of the strip edge and the strip dips a centimetre under that band, because the cut is the analytic q of a 40 km quad's interpolated position and jitters by millimetres — a cut exactly on the edge left a dotted line of uncovered pixels along the coast. Under water the sand is drawn saturated everywhere (it used to be dry and pale beyond the 26 m swash band, with a step where the band ended).

What lies on the sand is `coastBedCover` in `terrainShader.js` with the CPU twin `sampleSeabedCover` (also `surfaceAt().seabed`): weed meadows, silt and mussel banks as three masks of a metre-space sine field stretched along the shore, zoned by depth as a fraction of the authored depth — the surf keeps the first half metre bare, meadows take the middle of the shelf, silt settles in the calm deeper water, mussel banks sit as small dark islands. No new textures: the terrain material tints three resident layers (ground-fresh, loam, shells) and adds ragged edges, streaks and ripple marks (a tilt of the sand normal, crests 14 cm apart parallel to the shore, gone beyond 28 m and under weed) that the CPU twin leaves out. `coastBloom` reads the same meadows: the bloom is giant drifting masses plus patches, thicker over the weed; it tints the body, thickens the scattering, eats red and blue like chlorophyll, and lays scum lines along the wind on the surface. Controls in **Ландшафт → Дно → Шельф Азова**: `terrainShelfSlope`, `terrainWeed`, `terrainSilt`, `terrainMussels`, `terrainBedScale`, `terrainRipples`; the bloom stays in Суша → Ветер и среда. The material keys are excluded from the terrain query, so a slider move does not replant. `check:terrain` covers the drop and the cover; `check-terrain-gpu.html` compares the GPU cover with the twin (measured error 4e-6). The frame cost was within noise on the beach, along-coast, seaward and overview frames (11.7 vs 12.0 ms average on the surf frame). `check-water-surface-gpu.html` has been failing at its «left edge» probe since before this work (the page dates from the first coast commit and the shader changed four times after it).

Procedural detail is filtered by its pixel footprint, or it shimmers as the camera moves: from a hundred metres up the swell crests and the metre-scale bed noise fall below a few pixels per period, and a camera move changes every pixel — a moiré over the whole sea that read as the water flickering. `farWaterCrest` averages each swell cosine out once the phase changes more than 0.35 rad per pixel (gone at 1.5), for the far field and the strip alike; the bed's ragged edges, streaks, mussel speckle and ripple marks settle to their mean once a pixel spans more than 0.15–0.8 m of bed; the bloom's scum lines fade the same way. With a static camera consecutive frames differ only along the surf; the check was a 3 px camera move, diffed with sharp.

## The ground of the bluff

The bluff is coloured in tiers by height up the face, 0 at the foot and 1 at the crown as `coastHeight` lowers it on slides and ravines (`terrainMaterial.js`, the rock branch): the cold dark humus cap with the damp band under the turf, the loess with thin pale beds and rain rills the whole height, the loose talus at the foot. The loam texture stays one ochre; the shader recolours it exactly at the texture's mean (#91673D) to the daylight soils of the Taganrog shore from Denis's references: fresh scarp #C4A472 in patches on the steep faces, weathered crust #9A7C58 as the rule, humus #5F4C38, dry talus #C6AC80. `terrainWeathering` turns fresh into crust. The beds and rills also tilt the wall's normal in the two vertical triplanar projections and fade with distance so they cannot shimmer from afar; `terrainStrata` scales both. Ground and loam meet in a height blend, not a fade: the lumps stand out of the sand.

The ground at the foot is one field the material and the plants share: `coastSoil` in `terrainShader.js` with the CPU twin `sampleCoastSoil` in `terrainModel.js` (also `surfaceAt().talus` and `.seep`). x is the tongue of loose loam run out of the bluff onto the beach, lobed along the shore by `coastPatch` and blended over the sand by height so the shells show through its damp fringe; y is the seep where the bluff drains - its foot, the toes of slides, the mouths of ravines across the beach. Seep darkens the ground and lowers roughness the way the swash does, without foam, and keeps the cover green: `vegetation.dryness` is `terrainWeathering*(1-seep*.5)` and the shader's turf dryness follows it. `terrainTalus` (a geometry key) scales the run-out and the deposited cone of a slide, which peaks at the foot and spills onto the beach, gated to zero within four metres of the waterline so the surf and the shelf branch stay continuous. Controls: Суша → Поверхность → «Осыпь и языки грунта», «Пласты и потёки».
...
`terrainRocksEnabled`, `terrainRockSize` and `terrainDebris` switch and scale the boulders and the debris (the debris count no longer follows the boulder density); `terrainPebblesEnabled` gates the pebbles. In the editor they are the «Камни» and «Галька» nodes of the landscape group, and the laboratory shows the same geometry and materials as collection 10.
