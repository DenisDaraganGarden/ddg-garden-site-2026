...
async function main() {
  ...
  const homeEditKey = findManifestEntryKey(manifest, 'src/pages/HomeEdit.jsx');

  assert(homeKey, 'Could not find Home manifest entry.');
  assert(homeEditKey, 'Could not find HomeEdit manifest entry.');
  ...
  const homeEditFiles = collectImportedFiles(manifest, homeEditKey);
  ...
  assert(homeEditFiles.some((file) => file.includes('three-editor')), 'Home editor should import the editor-only chunk.');
