...
const productFiles = new Map(
  ['features', 'components', 'plants', 'tanker', 'terrain', 'topiary', 'materials', 'fire']
...
for (const dir of labDirs) {
  ...
  const importsProduct = files.some((file) => /from '\.\.\/(features|components|plants|tanker|terrain|topiary|materials|fire)\//.test(readFileSync(file, 'utf8')));
...
for (const page of [
  ...
  'house-lab/HouseLab.jsx',
  'fire-lab/FireLab.jsx',
]) {
