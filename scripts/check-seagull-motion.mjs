...
import {
  ...
  LANDING_STATE,
  updateFlightAgents,
} from '../src/features/home-scene/creatures/seagullFlight.js';
import {
  PERCHED_SOLE_HEIGHT_METERS,
  scareLandingAgent,
} from '../src/features/home-scene/creatures/seagullLanding.js';
import {
  BOAT_LANDING_SPECS,
...
import {
  ...
  SEAGULL_POINTER_LAW,
} from '../src/features/home-scene/creatures/seagullPointerInteraction.js';
import {
...
function checkFlight(count, mode) {
  ...
  if (mode === 'flight') {
    ...
    assert.ok(
      routeExtents[FLIGHT_ROUTE.WATERLINE].minY > 0.2
        && routeExtents[FLIGHT_ROUTE.WATERLINE].maxY < 0.72,
      'waterline birds must stay above and close to the y=0 production water plane',
    );
    assert.ok(routeExtents[FLIGHT_ROUTE.HIGH].minY > 3.8, 'high bird must stay clearly above the flock');
    assert.ok(
...
function checkShadowLod() {
  ...
  const agents = [
    ...
    { index: 6, position: new THREE.Vector3(11, 3.4, 0), shotState: 'falling' },
  ];
  const options = {
    waterY,
