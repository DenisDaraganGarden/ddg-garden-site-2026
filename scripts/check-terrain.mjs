...
import { buildCoastRocks, attachRockCollisions, makeRockGeometry, COAST_ROCK_TYPES } from '../src/terrain/terrainRocks.js';
...
// Each rock is seated, drawn and collided by its own shape (rock.variant,
// terrainRocks.js): one instanced mesh per shape, each rock measured by its own.
const rockShapes=COAST_ROCK_TYPES.map((_,variant)=>makeRockGeometry({variant}));
const rockMeshes=rockShapes.map((geometry,variant)=>new THREE.InstancedMesh(geometry,new THREE.MeshBasicMaterial(),rocks.filter(rock=>(rock.variant??0)===variant).length));
const transform=new THREE.Object3D(),slots=rockShapes.map(()=>0),placed=[];
rocks.forEach((rock)=>{const variant=rock.variant??0;transform.position.set(rock.x,rock.y,rock.z);transform.rotation.set(...rock.rotation);transform.scale.set(...rock.scale);transform.updateMatrix();rockMeshes[variant].setMatrixAt(slots[variant],transform.matrix);placed.push([variant,slots[variant]++]);});
const rockPoint=new THREE.Vector3(),rockMatrix=new THREE.Matrix4();
rocks.forEach((rock,index)=>{
  const [variant,slot]=placed[index],shape=rockShapes[variant].attributes.position;
  rockMeshes[variant].getMatrixAt(slot,rockMatrix);let lowest=Infinity;
  for(let i=0;i<shape.count;i++)lowest=Math.min(lowest,rockPoint.fromBufferAttribute(shape,i).applyMatrix4(rockMatrix).y);
  assert.ok(lowest<=sampleTerrainHeight(rock.x,rock.z,p)+.0001,'Bluff contact must not lift the slab above its beach footing');
});
rockMeshes.forEach(mesh=>{mesh.updateMatrixWorld();mesh.computeBoundingSphere();});
const raycaster=new THREE.Raycaster();
for(const rock of rocks.slice(0,30)){
  raycaster.set(new THREE.Vector3(rock.x,40,rock.z),new THREE.Vector3(0,-1,0));
  const hit=raycaster.intersectObjects(rockMeshes)[0];
...
rockMeshes.forEach(mesh=>{mesh.geometry.dispose();mesh.material.dispose();});
