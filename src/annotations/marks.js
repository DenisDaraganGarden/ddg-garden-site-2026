...
// у числа белый ореол, как у линий, — читается и так. line — толщина линий
// (множитель); поля холста растут с ней, остриё — ровно в точке (tipY).
export function drawMark(canvas, { text, color, level = 0, selected = false, zero = false, seed = 1, fill = true, outline = true, line = 1 }) {
    const ctx = canvas.getContext('2d');
    const font = `bold 19px ${FONT}`;
    ctx.font = font;
    const width = Math.ceil(ctx.measureText(text).width);
    // Снизу вверх: остриё, выноска до полочки, над ней рамка с числом.
    const edge = Math.ceil(line + 1.5) + 1;
    const tipX = TIP + Math.max(0, edge - 4), shelfY = 34 + level * SHELF, W = tipX + width + 30, H = shelfY + 32 + edge;
    ...
    const rand = wobble(seed), baseY = H - edge, top = baseY - shelfY;
    if (selected) sketch(ctx, [[tipX - 8, baseY], [tipX + 8, baseY]], '#f2c14e', 3 * line, rand);
    // Стрелка остриём в точку, выноска вверх, полочка вправо.
    sketch(ctx, [[tipX - 7, baseY - 11], [tipX, baseY], [tipX + 7, baseY - 11]], color, 2 * line, rand);
    sketch(ctx, [[tipX, baseY], [tipX, top]], color, 1.6 * line, rand);
    sketch(ctx, [[tipX, top], [tipX + width + 22, top]], color, (zero ? 2.4 : 1.6) * line, rand);
    if (zero) sketch(ctx, [[tipX + 4, top + 4], [tipX + width + 18, top + 4]], color, 1.1 * line, rand);
    ...
    if (outline) sketch(ctx, [[box.x + 5, box.y], [box.x + box.w - 5, box.y + 0.5], [box.x + box.w, box.y + 6], [box.x + box.w - 0.5, box.y + box.h - 5], [box.x + box.w - 6, box.y + box.h], [box.x + 5, box.y + box.h - 0.5], [box.x, box.y + box.h - 6], [box.x + 0.5, box.y + 5], [box.x + 5, box.y]], selected ? '#d19a2a' : color, 1.2 * line, rand);
    ...
    return { width: W, height: H, tipX, tipY: edge };
}
