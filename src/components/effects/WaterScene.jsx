  ...
  return (
    <CloudSceneContext.Provider value={cloudSceneRef}>
      ...
      >
        ...
        {settings.lightingEnabled && settings.lightingFixtures?.length ? <GardenLighting settings={settings} night={lighting.sky.night} selectedId={mode === 'editor' ? editorGizmo?.lighting?.selectedId : null} /> : null}
      ...
      {mode === 'editor' && settings.lightingEnabled && (settings.lightingFixtures?.length || settings.lightingPanels?.length) ? <ConnectionsLayer settings={settings} geometryKey={annotationGeometryKey} show={Boolean(editorGizmo?.lighting?.connections)} modelsReady={(settings.placedObjects ?? []).filter((o) => o.kind === 'model' && !o.hidden && settings.sketchupModels?.[o.id]).every((o) => loadedModels.split(',').includes(o.id))} /> : null}
