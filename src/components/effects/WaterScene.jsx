  ...
  return (
    <CloudSceneContext.Provider value={cloudSceneRef}>
      ...
      >
        ...
        {settings.plantingEnabled && (settings.plantingBeds?.length || settings.plantingPoints?.length || settings.plantingVines?.length) ? <PlantingLayer settings={settings} lowPower={qualityProfile.isLowPower} terrainQuery={settings.shoreEnabled ? terrainQuery : null} selectedBedId={mode === 'editor' ? editorGizmo?.planting?.selectedId : null} selectedVineId={mode === 'editor' ? editorGizmo?.planting?.vineId : null} envMapIntensity={lighting.environment.reflection} keyDirection={lighting.key.direction} /> : null}
