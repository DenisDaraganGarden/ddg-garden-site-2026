...
export const SPRAY_POOL = 16384;
export const SPRAY_TIERS = Object.freeze({
  low: { max: 2000, overdraw: 0.3, taps: 0, far: [70, 110], curl: 0 },
  medium: { max: 6000, overdraw: 0.6, taps: 1, far: [95, 150], curl: 0.35 },
  high: { max: 14000, overdraw: 1, taps: 1, far: [110, 170], curl: 0.35 },
});
const GRAVITY = 9.81;
...
export const SPRAY_RADIUS = 0.025;
export const SPRAY_GROW = 0.16;
...
export function sprayInstanceCount({ distance, height, viewportHeight, viewportWidth, projectionY, overdraw, max, minPx = 2.5, radius = SPRAY_RADIUS + SPRAY_GROW * 0.5 }) {
  ...
  return Math.min(Math.max(Math.round(budget / (quadPx * quadPx * 0.785)), 60), max);
}
...
export const sprayVertexBody = /* glsl */`
  ...
  if (weight < sprayHash(seed + 4.1) * 1.1) { gl_Position = vec4(2.0, 2.0, 2.0, 1.0); return; }
  ...
  float radius = uSprayRadius + uSprayGrow * span;
  float opacity = weight * smoothstep(0.0, 0.06, age) * (1.0 - smoothstep(0.72, 1.0, span));
  // A mote that has fallen back into the water is gone; the level is known
  // from the profile, so no depth texture is needed.
  opacity *= smoothstep(0.0, 0.30, world.y - waterY);
  ...
  vDetail = 1.0 - smoothstep(2.5, 8.0, pxRadius);
...
export function createSprayUniforms() {
  return {
    ...
    uSprayScale: { value: 0.5 },
    uSprayExtinction: { value: 9 },
    uSprayTaps: { value: 1 },
  };
}
...
export function syncSprayUniforms(uniforms, settings, tier = SPRAY_TIERS.high) {
  ...
  uniforms.uSprayScale.value = Number(settings.sprayGrain ?? 0.5);
  uniforms.uSprayExtinction.value = Number(settings.sprayDensity ?? 9);
