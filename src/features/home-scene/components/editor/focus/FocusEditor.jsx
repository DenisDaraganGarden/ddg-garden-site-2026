...
function FocusShell(props) {
    ...
    useLayoutEffect(() => {
        ...
        root.dataset.focusEditor = 'true'; root.dataset.focusPreview = String(preview && !playing); root.dataset.focusCanvas = String(focus || playing); root.dataset.focusFrame = solidFrame ? 'solid' : 'soft'; root.dataset.focusProject = String(Boolean(props.project));
        ...
        return () => { delete root.dataset.focusEditor; delete root.dataset.focusPreview; delete root.dataset.focusCanvas; delete root.dataset.focusFrame; delete root.dataset.focusProject; ['--focus-inspector-width', '--focus-panel-space', '--focus-rail-space'].forEach((name) => root.style.removeProperty(name)); };
    }, [width, collapsed, focus, preview, playing, hidden, solidFrame, props.project]);
    // Кадр проекта — на весь экран, без интерфейса сайта: окно (и приложение)
    // уходит в полноэкранный режим, Esc возвращает к редактору.
    const projectPreview = Boolean(props.project) && preview;
    useEffect(() => {
        if (!projectPreview || !document.fullscreenEnabled) return undefined;
        const leave = () => { if (!document.fullscreenElement) setPreview(false); };
        document.documentElement.requestFullscreen?.().catch(() => { /* the frame still fills the window */ });
        document.addEventListener('fullscreenchange', leave);
        return () => {
            document.removeEventListener('fullscreenchange', leave);
            if (document.fullscreenElement) document.exitFullscreen?.().catch(() => { /* already left */ });
        };
    }, [projectPreview]);
    ...
    useEffect(() => {
        const keyboard = (event) => {
            ...
            if (event.key === 'Escape') { setNavOpen(false); setViewsOpen(false); setPreview(false); }
