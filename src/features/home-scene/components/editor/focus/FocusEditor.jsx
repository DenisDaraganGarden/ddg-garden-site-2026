...
const ALL_TOOLS = [...TOOLS, ...PLANT_TOOLS, MARK_TOOL];
...
const PIE_ORDER = ['select', 'translate', 'rotate', 'scale', 'hand', 'greenery', 'mark'];
...
function FocusShell(props) {
    const { settings, activeTab, setActiveTab, layoutEditor: cameraEditor, gizmo, history, onPublish, onDeploy, onAdoptPublished, publishState, hasPublishChanges, publishEnabled, publishHint, playing = false, onPlay } = props;
    ...
    // Короткий пробел — пауза, как раньше; долгий — круг инструментов (FocusToolPie).
    const pauseState = useRef(); pauseState.current = { paused: settings.animationPaused, change: props.handleSettingChange };
    const togglePause = useCallback(() => pauseState.current.change({ target: { checked: !pauseState.current.paused } }, 'animationPaused', 'boolean'), []);
    ...
    const commands = [{ id: 'command:save', label: props.project ? tr('На заглавную', 'To the home page') : tr('В проект', 'Save to project'), icon: 'upload', action: () => setModal('save') }, { id: 'command:presets', label: tr('Детали объекта', 'Parts of this object'), icon: 'folder', action: () => setModal({ kind: 'presets', path: selected.path, label: t(`homeEditor.nodes.${selected.node.id}`) }) }, { id: 'command:settings', label: tr('Настройки движка', 'Engine settings'), icon: 'settings', action: () => setModal('settings') }, { id: 'command:help', label: tr('Горячие клавиши', 'Keyboard shortcuts'), icon: 'help', action: () => setModal('help') }, ...(onPlay ? [{ id: 'command:play', label: tr('Играть на доске', 'Ride the board'), trail: 'P', icon: 'water', action: onPlay }] : [])];
    ...
    return <div className={`focus-editor ${hidden ? 'focus-editor--hidden' : ''}`}>
        ...
        <div className="focus-stage-toolbar"><div className="focus-glass"><Button label={t('homeEditor.controls.technicalFrames')} onClick={() => setViewsOpen((value) => !value)}><FocusIcon name="camera" />{tr('Виды', 'Views')}<FocusIcon name="chevron" /></Button>{viewsOpen ? <FocusTechnicalViews settings={settings} layoutEditor={layoutEditor} frame={{ solid: solidFrame, toggle: () => setSolidFrame((value) => !value) }} /> : null}</div><span className="focus-spacer" /><NorthCompass settings={settings} applySettings={props.applySettings} tr={tr} /><span className="focus-spacer" /><div className="focus-glass"><Button icon="undo" label={tr('Отменить · ⌘Z', 'Undo · ⌘Z')} disabled={!history?.canUndo} onClick={history?.undo} /><Button icon="redo" label={tr('Повторить · ⌘⇧Z', 'Redo · ⌘⇧Z')} disabled={!history?.canRedo} onClick={history?.redo} /></div></div>
        ...
        <footer className="focus-statusbar"><span>{t(`homeEditor.groups.${selected.group.id}`) === t(`homeEditor.nodes.${selected.node.id}`) ? t(`homeEditor.nodes.${selected.node.id}`) : `${t(`homeEditor.groups.${selected.group.id}`)} / ${t(`homeEditor.nodes.${selected.node.id}`)}`}</span><span className="focus-spacer" />{publishState?.message || publishHint ? <span role="status">{publishState?.message || publishHint}</span> : null}<span>{(() => { const item = ALL_TOOLS.find((entry) => entry.id === gizmo?.tool); if (!item) return tr('Пробел — пауза, держать — инструменты', 'Space — pause, hold — tools'); const axes = item.transform && gizmo?.movable ? describeGizmoAxes(gizmo.movable, item.id, language) : ''; return [`${tr(item.ru, item.en)} · ${item.key}`, axes].filter(Boolean).join(' · '); })()}</span></footer>
        ...
        <FocusToolPie language={language} enabled={!playing && !modal} current={gizmo?.tool} onTap={togglePause}
