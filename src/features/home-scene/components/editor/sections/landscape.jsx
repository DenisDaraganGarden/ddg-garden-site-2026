...
const SeaRange = ({ settings, handleSettingChange, setting, ru, en, language, unit = '' }) => {
    ...
    return <RangeControl controlId={setting} label={seaLabel(ru, en, language)} value={settings[setting]} min={min} max={max} step={step} unit={unit} formatValue={(value) => Number(Number(value).toFixed(step < 0.1 ? 2 : 1))} onChange={(event) => handleSettingChange(event, setting)} />;
};
const SEA_WAVES = [
...
const SEA_SPRAY = [
    ['seaSprayAmount', 'Количество капель', 'Drop count'], ['seaSpraySize', 'Размер капель', 'Drop size', 'x'], ['seaSprayStreak', 'Вытянутость капель', 'Drop streaks'], ['seaSprayCurtain', 'Завеса с губы', 'Lip curtain'], ['seaSprayLife', 'Время полёта', 'Flight time', 's'],
    ['seaSprayDensity', 'Плотность капель', 'Drop density'], ['seaSpraySpread', 'Разброс вдоль гребня', 'Spread along the crest', 'm'],
];
const SEA_PUFFS = [
    ['seaSprayPuffSplash', 'Облачка всплеска', 'Splash puffs'], ['seaSprayPuffTrail', 'Облачка за волной', 'Puffs behind the wave'], ['seaSprayPuffSize', 'Размер облачков', 'Puff size', 'x'], ['seaSprayPuffDensity', 'Плотность облачков', 'Puff density'],
    ['seaSprayPuffLife', 'Жизнь облачков', 'Puff life', 's'], ['seaSprayPuffLift', 'Подъём облачков', 'Puff rise', 'm/s'], ['seaSprayPuffGrow', 'Разрастание облачков', 'Puff growth'],
];
const SEA_FOAM = [
    ['seaFoamLife', 'Живёт на воде', 'Lives on water', 's'], ['seaFoamDeposit', 'Плотность пены', 'Foam density'], ['seaFoamWindow', 'Окно памяти', 'Memory window', 'm'], ['seaFoamDrift', 'Снос ветром', 'Wind drift', 'm/s'], ['seaFoamSwirl', 'Завихрения', 'Swirl'], ['seaFoamDry', 'Сохнет песок', 'Sand dries in', 's'], ['seaSwashFilm', 'Плёнка заплеска', 'Swash film', 'm'], ['seaFoamThreshold', 'Порог пены', 'Foam threshold'], ['seaFoamSoftness', 'Мягкость', 'Softness'], ['seaFoamLaceScale', 'Масштаб кружева', 'Lace scale'], ['seaFoamVariety', 'Фактура пены', 'Foam texture'], ['seaFoamBrightness', 'Яркость пены', 'Foam brightness'], ['seaRipple', 'Рябь', 'Ripple'], ['seaWindPatches', 'Пятна ветра', 'Wind patches'], ['seaRippleScale', 'Масштаб ряби', 'Ripple scale'],
];
...
export const WaterWavesSection = ({ settings, handleSettingChange, applySettings }) => {
    ...
    return (
        <>
            ...
            <SectionHeading label={seaLabel('Облачка брызг', 'Spray puffs', language)} subtle />
            {SEA_PUFFS.map(([setting, ru, en, unit]) => <SeaRange key={setting} settings={settings} handleSettingChange={handleSettingChange} setting={setting} ru={ru} en={en} language={language} unit={unit} />)}
            ...
            />
            <RangeControl controlId={'waveLength'}
            ...
            />
            <RangeControl controlId={'rippleRadius'}
            ...
            />
            <RangeControl controlId={'rippleImpulse'}
...
export const WaterShaderSection = ({ settings, handleSettingChange }) => {
    ...
    return <>
        ...
        <CheckboxControl controlId={'seaFoamMemory'} label={seaLabel('Память пены', 'Foam memory', language)} checked={Boolean(settings.seaFoamMemory)} onChange={(event) => handleSettingChange(event, 'seaFoamMemory', 'boolean')} />
        ...
        <ColorControl controlId={'seaWaterColor'} label={seaLabel('Цвет воды', 'Water colour', language)} value={settings.seaWaterColor} onChange={(event) => handleSettingChange(event, 'seaWaterColor', 'color')} />
        <ColorControl controlId={'seaDeepColor'} label={seaLabel('Цвет глубины', 'Deep colour', language)} value={settings.seaDeepColor} onChange={(event) => handleSettingChange(event, 'seaDeepColor', 'color')} />
        <ColorControl controlId={'seaBedColor'} label={seaLabel('Цвет дна', 'Bed colour', language)} value={settings.seaBedColor} onChange={(event) => handleSettingChange(event, 'seaBedColor', 'color')} />
        ...
        <RangeControl controlId={'waterDepthMeters'}
        ...
        />
        <ColorControl controlId={'waterScatteringColor'}
...
export const SeabedSection = ({ settings, handleSettingChange }) => {
    ...
    return (
        <>
            ...
            <RangeControl controlId={'seabedReliefStrength'}
            ...
            />
            <RangeControl controlId={'seabedBrightness'}
            ...
            />
            <RangeControl controlId={'seabedVariation'}
            ...
            />
            <RangeControl controlId={'seabedAoStrength'}
            ...
            />
            <RangeControl controlId={'causticsSharpness'}
