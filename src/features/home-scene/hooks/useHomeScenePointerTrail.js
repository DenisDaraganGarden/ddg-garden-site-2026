...
import {
    ...
    createInactiveHomeSceneFieldData,
} from '../shaders/homeSceneFieldShader';

const createTrailBuffer = () => ({
    data: createInactiveHomeSceneFieldData(HOME_SCENE_FIELD_MAX_POINTS),
    currentPathLength: 0,
    pointCount: 0,
});
...
export const useHomeScenePointerTrail = ({
    radiusWorld,
    trailSpanWorld,
    trailPersistence,
    isDragging,
    ...
    useFrame((_, delta) => {
        ...
        samplesRef.current = samplesRef.current
            ...
            .slice(0, HOME_SCENE_FIELD_MAX_POINTS);
        ...
        const { data } = buffer;
        data.fill(0);
        for (let i = 0; i < HOME_SCENE_FIELD_MAX_POINTS; i += 1) {
            data[(i * HOME_SCENE_FIELD_STRIDE) + 2] = -1;
        }

        samplesRef.current.forEach((sample, index) => {
            const offset = index * HOME_SCENE_FIELD_STRIDE;
        ...
        writeDebugState({
            ...
            trailStride: HOME_SCENE_FIELD_STRIDE,
