export const HOME_SCENE_FIELD_MAX_POINTS = 64;
export const HOME_SCENE_FIELD_STRIDE = 4;
...
export const createInactiveHomeSceneFieldData = (maxPoints = HOME_SCENE_FIELD_MAX_POINTS) => {
    const data = new Float32Array(maxPoints * HOME_SCENE_FIELD_STRIDE).fill(0);

    for (let i = 0; i < maxPoints; i += 1) {
        data[(i * HOME_SCENE_FIELD_STRIDE) + 2] = -1;
    }

    return data;
};

export const buildHomeSceneFieldUniforms = (maxPoints = HOME_SCENE_FIELD_MAX_POINTS) => ({
    uTrailData: { value: createInactiveHomeSceneFieldData(maxPoints) },
...
export const getHomeSceneFieldConfig = (settings = {}) => {
...
export const syncHomeSceneFieldUniforms = (uniforms, config, trailBuffer, time) => {
    if (!uniforms || !config) {
        return;
    }
...
export const buildHomeSceneFieldGLSL = (maxPoints = HOME_SCENE_FIELD_MAX_POINTS) => `
uniform float uTrailData[${maxPoints * HOME_SCENE_FIELD_STRIDE}];
...
float getTrailDisplacement(
    vec2 worldXZ,
    float trailData[${maxPoints * HOME_SCENE_FIELD_STRIDE}],
    float currentTime,
    ...
    for (int i = 0; i < ${maxPoints - 1}; i += 1) {
        ...
        int offsetA = i * ${HOME_SCENE_FIELD_STRIDE};
        int offsetB = (i + 1) * ${HOME_SCENE_FIELD_STRIDE};
...
const float GLSL_TRAIL_FIELD_STRIDE = ${HOME_SCENE_FIELD_STRIDE}.0;

vec4 getMorphingTexture(
    vec2 uv,
    sampler2D tex1,
    sampler2D tex2,
    float morph,
    ...
    vec2 distortedUv = uv + (displacement * parallaxStrength * (uv - vec2(0.5)));
    vec4 col1 = texture(tex1, distortedUv);
    vec4 col2 = texture(tex2, distortedUv);
    return mix(col1, col2, morph);
}
`;
