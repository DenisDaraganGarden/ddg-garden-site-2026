...
const DEFAULTS = Object.freeze({
  ...
  foamMemory: true, foamLife: 7, foamDeposit: 0.9, foamWindow: 200, foamDrift: 0.9, foamDry: 43, swashFilm: 0.03,
...
const PRESETS = {
  calm: { amplitude: 0.16, steepness: 0.3, gusts: 0.2, foamThreshold: 0.35, foamDeposit: 0.5, foamLife: 4, volumeHeight: 0.12, volumeDensity: 0.7, volumeErosion: 0.55, surfHeight: 0.3 },
  breeze: { amplitude: 0.42, steepness: 0.5, gusts: 0.4, foamThreshold: 0.5, foamDeposit: 0.75, foamLife: 6, volumeHeight: 0.22, volumeDensity: 0.9, volumeErosion: 0.45, surfHeight: 0.6 },
  rough: { amplitude: 0.75, steepness: 0.68, gusts: 0.6, foamThreshold: 0.62, foamDeposit: 0.95, foamLife: 9, volumeHeight: 0.38, volumeDensity: 1.15, volumeErosion: 0.32, surfHeight: 1.1 },
  storm: { amplitude: 1.25, steepness: 0.8, gusts: 0.85, foamThreshold: 0.78, foamDeposit: 1.3, foamLife: 14, volumeHeight: 0.6, volumeDensity: 1.5, volumeErosion: 0.2, surfHeight: 1.8 },
};
const VIEWS = {
  eye: { landscape: { position: at(-14, ALONG_MID + 8, 1.5), target: at(-30, ALONG_MID, 0.4) }, portrait: { position: at(-16, ALONG_MID + 10, 1.8), target: at(-30, ALONG_MID, 0.4) } },
  macro: { landscape: { position: at(-18, ALONG_MID + 3, 0.75), target: at(-24, ALONG_MID, 0.25) }, portrait: { position: at(-19, ALONG_MID + 4, 0.9), target: at(-24, ALONG_MID, 0.25) } },
  above: { landscape: { position: at(-30, ALONG_MID, 26), target: at(-16, ALONG_MID, 0) }, portrait: { position: at(-40, ALONG_MID, 34), target: at(-16, ALONG_MID, 0) } },
  far: { landscape: { position: at(6, ALONG_MID + 40, 4.5), target: at(-90, ALONG_MID - 20, 0.5) }, portrait: { position: at(8, ALONG_MID + 50, 5.5), target: at(-90, ALONG_MID - 20, 0.5) } },
};
...
const COPY = {
  ru: { title: 'Пена · объём по принципу облаков', subtitle: 'кусок воды с валом · пена как объёмная среда · штиль, бриз, волнение, шторм', volume: 'Объём', field: 'Поле', wave: 'Волна', light: 'Свет', eye: 'С воды', macro: 'Вблизи', above: 'Сверху', far: 'Вдаль', volumeOn: 'Объём включён', volumeHeight: 'Высота пены', volumeDensity: 'Плотность', volumeDetail: 'Масштаб комков', volumeErosion: 'Эрозия верха', volumeNear: 'Плоская с', volumeFar: 'Плоская к', brightness: 'Яркость пены', threshold: 'Порог пены', softness: 'Мягкость', deposit: 'Осаждение', life: 'Живёт на воде', window: 'Окно памяти', drift: 'Снос ветром', lace: 'Масштаб кружева', amplitude: 'Высота волны', wavelength: 'Длина волны', steepness: 'Крутизна', gusts: 'Порывы', speed: 'Скорость', wind: 'Направление ветра', surfHeight: 'Высота вала', surfEnabled: 'Вал включён', time: 'Время суток', bearing: 'Направление солнца', elevation: 'Высота солнца', exposure: 'Экспозиция', pause: 'Пауза', play: 'Продолжить', reset: 'Сброс', calm: 'Штиль', breeze: 'Бриз', rough: 'Волнение', storm: 'Шторм', tri: 'треугольников', calls: 'вызовов', fps: 'кад/с', assets: 'Коллекции', metres: 'м', hours: 'ч', seconds: 'с', mps: 'м/с' },
  en: { title: 'Foam · volume by the clouds’ principle', subtitle: 'a patch of water with a breaker · foam as a medium · calm, breeze, rough, storm', volume: 'Volume', field: 'Field', wave: 'Wave', light: 'Light', eye: 'From the water', macro: 'Close', above: 'Above', far: 'Distance', volumeOn: 'Volume on', volumeHeight: 'Foam height', volumeDensity: 'Density', volumeDetail: 'Lump scale', volumeErosion: 'Top erosion', volumeNear: 'Flat from', volumeFar: 'Flat by', brightness: 'Foam brightness', threshold: 'Foam threshold', softness: 'Softness', deposit: 'Deposit', life: 'Lives on water', window: 'Memory window', drift: 'Wind drift', lace: 'Lace scale', amplitude: 'Wave height', wavelength: 'Wavelength', steepness: 'Steepness', gusts: 'Gusts', speed: 'Speed', wind: 'Wind direction', surfHeight: 'Breaker height', surfEnabled: 'Breaker on', time: 'Time of day', bearing: 'Sun bearing', elevation: 'Sun elevation', exposure: 'Exposure', pause: 'Pause', play: 'Resume', reset: 'Reset', calm: 'Calm', breeze: 'Breeze', rough: 'Rough', storm: 'Storm', tri: 'triangles', calls: 'draw calls', fps: 'fps', assets: 'Collections', metres: 'm', hours: 'h', seconds: 's', mps: 'm/s' },
};
...
export default function FoamLab() {
  ...
  return <main className="water-lab" data-asset-collection="foam" lang={language}>
    ...
    <div className="water-lab__workspace"><section className="water-lab__viewer" aria-label="Foam viewport">
      ...
      <div className="water-lab__views" role="group" aria-label="Ракурс">{['eye', 'macro', 'above', 'far'].map((id) => <button key={id} aria-pressed={view === id} onClick={() => setView(id)}>{t[id]}</button>)}</div>
      ...
      <div className="water-lab__controls" role="tabpanel" aria-label={t[tab]}>
        {tab === 'volume' && <><Toggle label={t.volumeOn} value={settings.volume} onChange={(value) => set('volume', value)} />{range('volumeHeight', t.volumeHeight, 0, 1.2, 0.01, t.metres)}{range('volumeDensity', t.volumeDensity, 0.2, 2.5, 0.05)}{range('volumeDetail', t.volumeDetail, 0.2, 3, 0.05)}{range('volumeErosion', t.volumeErosion, 0, 0.9, 0.01)}{range('volumeNear', t.volumeNear, 20, 400, 5, t.metres)}{range('volumeFar', t.volumeFar, 40, 800, 5, t.metres)}{range('foamBrightness', t.brightness, 0.2, 2, 0.05)}</>}
        {tab === 'field' && <>{range('foamThreshold', t.threshold, 0, 0.95, 0.01)}{range('foamSoftness', t.softness, 0.02, 0.4, 0.01)}{range('foamDeposit', t.deposit, 0.2, 1.5, 0.05)}{range('foamLife', t.life, 1, 20, 0.5, t.seconds)}{range('foamWindow', t.window, 32, 400, 4, t.metres)}{range('foamDrift', t.drift, 0, 2, 0.05, t.mps)}{range('laceScale', t.lace, 0.03, 0.6, 0.01)}</>}
        {tab === 'wave' && <>{range('amplitude', t.amplitude, 0, 1.6, 0.01, t.metres)}{range('wavelength', t.wavelength, 3, 40, 0.5, t.metres)}{range('steepness', t.steepness, 0, 0.8, 0.01)}{range('gusts', t.gusts, 0, 1, 0.01)}{range('speed', t.speed, 0, 2.5, 0.05)}{range('windDirection', t.wind, 0, 360, 1, '°')}<Toggle label={t.surfEnabled} value={settings.surfEnabled} onChange={(value) => set('surfEnabled', value)} />{range('surfHeight', t.surfHeight, 0.2, 3, 0.05, t.metres)}</>}
