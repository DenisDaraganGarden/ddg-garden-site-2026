...
const VIEWS = {
  ...
  surf: { landscape: { position: at(-4, ALONG_MID + 14, 2.2), target: at(-34, ALONG_MID, 0.6) }, portrait: { position: at(-2, ALONG_MID + 18, 2.6), target: at(-34, ALONG_MID, 0.6) } },
};
...
export default function FoamLab() {
  ...
  return <main className="water-lab" data-asset-collection="foam" lang={language}>
    ...
    <div className="water-lab__workspace"><section className="water-lab__viewer" aria-label="Foam viewport">
      ...
      <div className="water-lab__views" role="group" aria-label="Ракурс">{['eye', 'macro', 'above', 'far', 'surf'].map((id) => <button key={id} aria-pressed={view === id} onClick={() => setView(id)}>{t[id]}</button>)}</div>
