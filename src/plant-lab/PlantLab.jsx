...
import LabShell,{LabColor,LabModes,LabRange,LabSelect,LabTabs,LabToggle} from '../asset-lab/LabShell';
...
export default function PlantLab({species=PLANT_SPECIES.oleaster,variants,onVariant}){
 ...
 const range=(key,label,min=0,max=1,step=.01,unit='')=><LabRange key={key} label={label} value={settings[key]} min={min} max={max} step={step} unit={unit} format={v=>Number(v).toFixed(step>=1?0:step>=.1?1:2)} onChange={v=>set(key,v)}/>;
 const selectMode=next=>{setMode(next);setView(next==='patch'?'patch':'full');};
 return <LabShell
  collection={collection}
  testId={`${collection}-lab`}
  eyebrow={`DDG / ASSET LAB / ${assetIndex(collection)}`}
  title={t.title}
  subtitle={t.subtitle}
  language={lang}
  onLanguage={setLang}
  views={(mode==='patch'?['patch','base','far']:['full','macro','base','top','far']).map(v=>({id:v,label:t[v]}))}
  view={view}
  onView={setView}
  scale={`${stats.scale?.metres??.25} ${t.units}`}
  panel={<>
   {variants&&<LabModes label="Вид" items={variants.map(v=>({id:v.id,label:v[lang]}))} value={species.id} onChange={onVariant}/>}
   <LabModes label={t.specimen} items={['specimen','patch'].map(m=>({id:m,label:t[m]}))} value={mode} onChange={selectMode}/>
   <LabTabs label={t[tab]} items={['plant','wind','surface','ground','light'].map(id=>({id,label:t[id]}))} value={tab} onChange={setTab}/>
   {tab==='plant'&&<>{species.sliders.map(([key,min,max,step,unit])=>range(key,t[key],min,max,step,unit==='m'?t.units:''))}<LabSelect label={t.lod} value={settings.lod} onChange={v=>set('lod',v)} options={['auto','0','1','2'].map((id,i)=>({value:id,label:[t.auto,t.near,t.middle,t.distant][i]}))}/><LabToggle label={t.skeleton} value={settings.skeleton} onChange={v=>set('skeleton',v)}/><LabToggle label={t.wire} value={settings.wireframe} onChange={v=>set('wireframe',v)}/><LabToggle label={t.mobile} value={lowPower} onChange={setLowPower}/></>}
  ...
  transport={<>
   <button type="button" aria-pressed={paused} onClick={()=>setPaused(p=>!p)}>{paused?'▶':'Ⅱ'} {paused?t.play:t.pause}</button>
   <button type="button" onClick={()=>{setSettings(DEFAULTS);setView('full');setMode('specimen');}}>{t.reset}</button>
  </>}
  stats={<>
   <span><b>{stats.triangles.toLocaleString()}</b> {t.tri}</span>
   <span><b>{stats.calls}</b> {t.calls}</span>
   <span>LOD <b>{stats.lods.join(' / ')}</b></span>
   <span><b>{stats.leaves}</b> {t.leaves} · <b>{stats.branches}</b> {t.branches}</span>
  </>}
 >
  <AssetStudio view={view} cameraViews={cameraViews} cameraLimits={LIMITS} cameraFar={species.cameraFar} fogRange={species.fogRange} floorY={-.014} floorVisible={mode==='specimen'} sceneOverrides={{timeOfDay:settings.timeOfDay}} shadowRadius={mode==='patch'?Math.max(9,settings.extent*.55):9} exposure={settings.exposure} environmentIntensity={1} paused={paused} inactive={hidden} pixelRatio={lowPower?1:[1,1.5]}>
