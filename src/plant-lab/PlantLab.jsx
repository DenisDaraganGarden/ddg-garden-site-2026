...
export default function PlantLab({species=PLANT_SPECIES.oleaster}){
 ...
 return <main className="plant-lab" data-asset-collection={species.id} lang={lang}>
  <header className="plant-lab__header"><div><p>DDG / ASSET LAB / {species.index}</p><h1>{t.title}</h1><span>{t.subtitle}</span></div><div className="plant-lab__header-actions"><div>{['ru','en'].map(l=><button key={l} aria-pressed={l===lang} onClick={()=>setLang(l)}>{l.toUpperCase()}</button>)}</div><nav><a href="?collection=river-fish">01</a><a href="?collection=seagulls">02</a><a href="?collection=black-stone-sculpture">03</a><a href="?collection=tanker">04</a><a href="?collection=oleaster" aria-current={species.id==='oleaster'?'page':undefined}>05</a><a href="?collection=tree" aria-current={species.id==='tree'?'page':undefined}>06</a></nav></div></header>
