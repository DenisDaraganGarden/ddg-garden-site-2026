...
const DEFAULTS={...OLEASTER_DEFAULTS,...ECOLOGY_DEFAULTS,dryness:.42,lod:'auto',wireframe:false,skeleton:false,count:160,extent:24,slope:.12,pathWidth:.7,light:'studio',timeOfDay:16,exposure:1.04};
const COPY={
...
export default function PlantLab(){
 ...
 const cameraViews=useMemo(()=>{
  ...
  return {...VIEWS,patch:{landscape:{position:[extent*.65,y+extent*.42,extent*.87],target:[0,y+.55,0]},portrait:{position:[extent*.8,y+extent*.55,extent*1.05],target:[0,y+.55,0]}},base:{landscape:rootView,portrait:rootView},far:{landscape:{position:[0,y+2,Math.max(65,extent*1.6)],target:[0,y+.55,0]},portrait:{position:[0,y+2,Math.max(80,extent*1.9)],target:[0,y+.55,0]}}};
 },[mode,settings.slope,settings.extent,focusX,focusY,focusZ]);
 ...
 return <main className="plant-lab" data-asset-collection="oleaster" lang={lang}>
  ...
  <div className="plant-lab__workspace"><section className="plant-lab__viewer" aria-label="3D plant viewport">
   ...
   <div className="plant-lab__controls" role="tabpanel" aria-label={t[tab]}>
    ...
    {tab==='wind'&&<>{range('wind',t.windSpeed,0,14,.1,lang==='ru'?'м/с':'m/s')}{range('windBearing',t.bearing,0,360,1,'°')}{range('flutter',t.flutter,0,1,.05)}{range('gustStrength',t.gustStrength,0,1,.05)}{range('lodging',t.lodging,0,1,.05)}</>}
    ...
    {tab==='ground'&&<>{range('count',t.count,1,512,1)}{range('extent',t.extent,8,64,1,t.units)}{range('fieldSeed',t.fieldSeed,1,200,1)}{range('slope',t.slope,0,.45,.01)}{range('pathWidth',t.path,0,2,.1,t.units)}</>}
