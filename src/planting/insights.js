...
export function scopeRows(scope, beds, fills, points, library) {
    const rows = new Map();
    const row = (id) => {
        if (!rows.has(id)) rows.set(id, { id, plant: library.get(id) ?? { id, ru: id, category: 'perennial' }, count: 0, area: 0 });
        return rows.get(id);
    };
    let area = 0;
    if (scope.kind === 'beds') {
    ...
    return { rows: list, count, species: list.length, area };
}
...
export const CATEGORY_ORDER = ['tree', 'conifer', 'topiary', 'shrub', 'grass', 'perennial', 'groundcover'];
