...
function GrassKind({kind,placements,asset,settings,lowPower}){
 ...
 return atlas&&placements.length?<group name={`coastal-grass-${kind}`}><PlantPopulation model={model} atlas={atlas} settings={renderSettings} placements={placements} sceneTime lowPower={lowPower} statsKey={`grassStats_${kind}`} impostorFrame={carpet?(lowPower?128:256):lowPower?96:192}/></group>:null;
}
