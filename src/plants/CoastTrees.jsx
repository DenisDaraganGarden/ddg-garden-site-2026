...
export default function CoastTrees({settings,plants,qualityProfile,envMapIntensity=1}){
 ...
 // The oleaster's leaf atlas and Denis's oleaster bark tile (half size on a phone).
 ...
 const shapeKey=JSON.stringify({seed:settings.seed,height:settings.height,spread:settings.spread,lean:settings.lean,twist:settings.twist,density:settings.density,leafSize:settings.leafSize,deadwood:settings.deadwood,windBearing:settings.windBearing,midEvery:lowPower?4:2,midSkipsThin:lowPower,barkTile:BARK_TILE.oleaster});
 ...
 return atlas&&placements.length?<group name="coastal-oleaster-trees"><PlantPopulation model={model} atlas={atlas} settings={renderSettings} placements={placements} sceneTime lowPower={lowPower} statsKey="treeStats" impostorFrame={lowPower?192:384} envMapIntensity={envMapIntensity}/></group>:null;
}
