...
import {GRASS_UNDERLAY,GRASS_SCENE_KINDS} from './grassModel.js';
...
const BASE_DENSITY={stipa:.55,festuca:.9,leymus:.12,phragmites:.35,carpet:1.6};
const SHARE_KEY={stipa:'grassStipa',festuca:'grassFestuca',leymus:'grassLeymus',phragmites:'grassPhragmites',carpet:'grassCarpet'};
...
function kindsAt(q,f,p){
 ...
 if(q>f.foot-1){out.push('festuca');out.push(GRASS_UNDERLAY);}
...
function suitability(kind,s,q,sc,p,f){
 ...
 switch(kind){
  ...
  case GRASS_UNDERLAY:return s.height>.2&&s.normal.y>.85?(.55+.45*(p.terrainSoil??.5))*smooth(f.foot-1,f.foot+2,q):0;
  case 'leymus':{
...
export function grassCell(cx,cz,{definition:p,query,settings}){
 ...
 const out=Object.fromEntries(GRASS_SCENE_KINDS.map(kind=>[kind,[]]));
 ...
 for(const kind of kinds){
  const share=settings[SHARE_KEY[kind]]??1;
  // The carpet is its own slider, not a share of the density.
  const expected=BASE_DENSITY[kind]*share*(kind===GRASS_UNDERLAY?(settings.lowPower?.5:1):settings.grassDensity)*GRASS_CELL*GRASS_CELL;
  ...
  for(let i=0;i<candidates;i++){
   ...
   const colonies=kind===GRASS_UNDERLAY?.6+.4*ecologyPatch(x,z,14,settings.grassSeed+5):.3+.7*ecologyPatch(x,z,32,settings.grassSeed+kind.length*7);
...
export function gatherGrass(cache,cameraX,cameraZ,radius,context,budgetMs=12,carpetRadius=radius){
 ...
 for(let dz=-span;dz<=span;dz++)for(let dx=-span;dx<=span;dx++){
  ...
  const nearEnough=dx*dx+dz*dz<=carpetSpan*carpetSpan;
  for(const kind of GRASS_SCENE_KINDS){if(kind===GRASS_UNDERLAY&&!nearEnough)continue;if(cell[kind]?.length)out[kind].push(...cell[kind]);}
 }
 return out;
}
