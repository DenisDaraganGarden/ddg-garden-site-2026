...
## Home scene integration

Reviewed asset source: `codex/asset-lab` commit `188aeb1`. `HomeTanker.jsx` places the physical model in the scene, chooses detail with 620/780 m hysteresis, and integrates movement in metres. Site bearing is clockwise from north (-Z). A long route repeats after its configured length; its end should be placed outside authored camera views. Scene normalization, publication and camera snapshots include the tanker fields.

`SoundscapeEngine.updateTanker()` lazily connects `TankerSound` to the existing world bus; `HomeSoundscapeBridge` retains the listener. The mixer supports tanker gain, mute, solo and horn preview. The source releases on scene removal. `scripts/check-tanker-integration.html` verifies the actual shared graph in OfflineAudioContext: half master produces half RMS, and master mute, music-only, other-track solo and disabled tanker produce silence.

Far water consumes the same planar reflection as the central surface. The moving tanker keeps that pass active; its wake is excluded from optical captures. The original scene and camera data are preserved.
