...
import { spitShader, SPIT_SEGMENTS } from './terrainSpit.js';
import { createTerrainDefinition } from './terrainModel.js';
...
${spitShader}
float coastOffshore(){return max(96.0,uCoastBed.z);}
...
float coastMask(vec2 qs) {
 return uCoastShape.x*smoothstep(-coastOffshore(),-coastOffshore()+24.0,qs.x)*(1.0-smoothstep(uCoastDimensions.y-48.0,uCoastDimensions.y,qs.x))*(1.0-smoothstep(uCoastDimensions.x*.5-64.0,uCoastDimensions.x*.5,abs(qs.y)));
}
...
float coastHeight(vec2 qs) {
 ...
 return coastSpitHeight(mix(-uCoastSurface.y,shelf+beach+cliff*bank*(1.0-slump)+talus+erosion+dryNoise+upland,coastMask(qs)),qs,coastMask(qs));
}
...
float coastWavePhase(vec2 qs,float time){qs=coastSurfLocal(qs);return qs.x*1.15-time*6.28318530718/uCoastSurf.y+qs.y*.015+coastWaveJitter(qs.y,time)*1.3;}
// The envelope: how high the water can climb at this point of the beach.
float coastWaveGain(vec2 qs,float time) {
 float coverage=coastMask(qs);qs=coastSurfLocal(qs);
 float q=qs.x,s=qs.y;
 ...
 return uCoastSurf.x*amp*shoal*(1.0-smoothstep(0.0,4.5,q))*smoothstep(-32.0,-22.0,q)*coverage;
}
float coastWave(vec2 qs,float time) {
 float q=coastSurfLocal(qs).x,phase=coastWavePhase(qs,time);
...
CoastSwash coastSwashAt(vec2 qs,float time,float ground) {
 ...
 float skew=.3*smoothstep(-18.0,-4.0,coastSurfLocal(qs).x);
...
vec2 coastFoamTerms(vec2 qs,vec3 world,float time,float ground) {
 float coverage=coastMask(qs);vec2 originalQS=qs;
 CoastSwash w=coastSwashAt(qs,time,ground);
 qs=coastSurfLocal(qs);float q=qs.x;
 float depth=w.depth-coastEdgeRag(originalQS,time);
 ...
 float crest=pow(max(0.0,sin(coastWavePhase(originalQS,time)-.45)),3.0);
 ...
 float span=coverage*smoothstep(-18.0,-2.0,q)*(1.0-smoothstep(5.0,9.0,q))*uCoastSurf.z*surfPatch;
