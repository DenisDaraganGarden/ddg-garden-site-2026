...
// water switched off: the swell and the breakers here are the new water.
const DEFINITION = createTerrainDefinition(PUBLISHED);
const TERRAIN_SETTINGS = Object.freeze({ ...PUBLISHED, waterVisible: false });
const ROCKS = buildCoastRocks(DEFINITION);
...
const DEFAULTS = Object.freeze({
  wavelength: 18, amplitude: 0.55, steepness: 0.7, speed: 1, windDirection: 95, sets: 0.6, gusts: 0.6, crossWaves: 0.2, fadeStart: 400, fadeEnd: 2500,
  ripple: 0.35, rippleScale: 0.09,
  foamThreshold: 0.42, foamSoftness: 0.08, laceScale: 0.14, foamBrightness: 1,
  foamMemory: true, foamLife: 8, foamDeposit: 0.9, foamWindow: 200, foamDrift: 0.4, foamDry: 40,
  waterColor: '#2c7a64', deepColor: '#143a40', bedColor: '#c4b08a', crestGlow: 0.7, glint: 1, skyReflection: 0.45,
  meshRings: 112, meshSegments: 144, wireframe: false,
  surfEnabled: true, surfHeight: 1.1, surfWidth: 9, surfBreakDistance: -12, surfBreakLength: 16, surfLean: 0.45, surfJet: 2.2, surfLift: 0.9, surfSheet: 0.16, surfRoller: 0.5, surfRollerDensity: 1, surfPeel: 0.06, surfRefraction: 0.7, surfBoreLength: 14, surfRunup: 6, surfSpeed: 4.5, surfPeriod: 9, surfSets: 0.5, surfFreeze: false, surfPhase: 0.5,
...
// The break line of the default breaker, for the cameras that look into the tube.
const BREAK_Q = breakLineMean(coastBreakLine(DEFINITION, DEFAULTS.surfHeight, ALONG0, CREST_LENGTH, 8));
const VIEWS = {
  ...
  macro: { landscape: { position: at(-48, ALONG_MID + 4.5, 1.7), target: at(-54, ALONG_MID, 0.2) }, portrait: { position: at(-46, ALONG_MID + 6, 2.3), target: at(-54, ALONG_MID, 0.2) } },
  surf: { landscape: { position: at(16, ALONG_MID + 6, 2.4), target: at(BREAK_Q - 2, ALONG_MID, 0.7) }, portrait: { position: at(20, ALONG_MID + 8, 3), target: at(BREAK_Q - 2, ALONG_MID, 0.7) } },
  surfSide: { landscape: { position: at(BREAK_Q + 2.5, ALONG_MID - 9, 1.3), target: at(BREAK_Q - 1, ALONG_MID, 0.7) }, portrait: { position: at(BREAK_Q + 3, ALONG_MID - 12, 1.6), target: at(BREAK_Q - 1, ALONG_MID, 0.7) } },
  lip: { landscape: { position: at(BREAK_Q + 9, ALONG_MID + 7, 1.2), target: at(BREAK_Q + 0.5, ALONG_MID, 0.55) }, portrait: { position: at(BREAK_Q + 11, ALONG_MID + 9, 1.5), target: at(BREAK_Q + 0.5, ALONG_MID, 0.55) } },
  surfAbove: { landscape: { position: at(-45, ALONG_MID, 38), target: at(BREAK_Q, ALONG_MID, 0) }, portrait: { position: at(-60, ALONG_MID, 55), target: at(BREAK_Q, ALONG_MID, 0) } },
...
export default function WaterLab() {
  ...
  const coast = useMemo(() => ({
    definition: DEFINITION, along0: ALONG0, length: CREST_LENGTH, shoreDepth, foamField,
  ...
  return <main className="water-lab" data-asset-collection="water" lang={language}>
    ...
    <div className="water-lab__workspace"><section className="water-lab__viewer" aria-label="Water viewport">
      <AssetStudio view={view} cameraViews={VIEWS} cameraLimits={LIMITS} cameraFar={6000} fogRange={[1200, 4500]} floorVisible={false} lighting={lighting} exposure={settings.exposure} environmentIntensity={1} paused={paused} inactive={hidden} pixelRatio={[1, 1.5]} background="#a9c8d9" shadowRadius={40}>
        ...
        <GerstnerWaterSurface settings={settings} lighting={lighting} noise={noise} wireframe={settings.wireframe} coast={coast} foamBores={settings.surfEnabled ? foamBores : null} />
        {settings.surfEnabled ? <BreakingWaves settings={settings} lighting={lighting} noise={noise} coast={coast} foamBores={foamBores} /> : null}
        <Suspense fallback={null}><AzovTerrain definition={DEFINITION} settings={TERRAIN_SETTINGS} qualityProfile={qualityProfile} lighting={lighting} sky={null} runtime={null} rocks={ROCKS} swash={foamField} /></Suspense>
        <LabStats onStats={setStats} />
      </AssetStudio>
      <div className="water-lab__views" role="group" aria-label="Ракурс">{['shore', 'above', 'macro', 'surf', 'surfSide', 'lip', 'surfAbove', 'spit'].map((id) => <button key={id} aria-pressed={view === id} onClick={() => setView(id)}>{viewLabel(id)}</button>)}</div>
      ...
      <div className="water-lab__controls" role="tabpanel" aria-label={t[tab]}>
        ...
        {tab === 'surf' && <><Toggle label={t.surfEnabled} value={settings.surfEnabled} onChange={(value) => set('surfEnabled', value)} /><Toggle label={t.surfFreeze} value={settings.surfFreeze} onChange={(value) => set('surfFreeze', value)} />{range('surfPhase', t.surfPhase, 0, 1, 0.01)}{range('surfHeight', t.surfHeight, 0.2, 3, 0.05, t.metres)}{range('surfWidth', t.surfWidth, 3, 24, 0.5, t.metres)}{range('surfBreakDistance', t.surfBreakDistance, -20, 40, 0.5, t.metres)}{range('surfBreakLength', t.surfBreakLength, 4, 40, 1, t.metres)}{range('surfLean', t.surfLean, 0, 1, 0.01)}{range('surfJet', t.surfJet, 0.3, 4, 0.05, t.mps)}{range('surfLift', t.surfLift, 0, 2, 0.05, t.mps)}{range('surfSheet', t.surfSheet, 0.04, 0.4, 0.01)}{range('surfRoller', t.surfRoller, 0, 1.2, 0.02)}{range('surfRollerDensity', t.surfRollerDensity, 0.2, 2.5, 0.05)}{range('surfPeel', t.surfPeel, 0, 0.6, 0.01)}{range('surfRefraction', t.surfRefraction, 0, 1, 0.01)}{range('surfBoreLength', t.surfBoreLength, 3, 40, 1, t.metres)}{range('surfRunup', t.surfRunup, 0, 30, 1, t.metres)}{range('surfSpeed', t.surfSpeed, 1, 10, 0.1, t.mps)}{range('surfPeriod', t.surfPeriod, 3, 20, 0.5, t.seconds)}{range('surfSets', t.surfSets, 0, 1, 0.01)}</>}
        {tab === 'foam' && <><Toggle label={t.foamMemory} value={settings.foamMemory} onChange={(value) => set('foamMemory', value)} />{range('foamLife', t.foamLife, 1, 20, 0.5, t.seconds)}{range('foamDeposit', t.foamDeposit, 0.2, 1.5, 0.05)}{range('foamWindow', t.foamWindow, 32, 400, 4, t.metres)}{range('foamDrift', t.foamDrift, 0, 2, 0.05, t.mps)}{range('foamDry', t.foamDry, 5, 120, 1, t.seconds)}{range('foamThreshold', t.threshold, 0, 0.95, 0.01)}{range('foamSoftness', t.softness, 0.02, 0.4, 0.01)}{range('laceScale', t.lace, 0.03, 0.6, 0.01)}{range('foamBrightness', t.brightness, 0.2, 2, 0.05)}{range('ripple', t.ripple, 0, 1, 0.01)}{range('rippleScale', t.rippleScale, 0.01, 0.3, 0.005)}</>}
