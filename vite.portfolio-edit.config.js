...
function portfolioEditorAtRootPlugin() {
  const redirectRootToEditor = (middlewares) => {
    middlewares.use((request, response, next) => {
      if (request.url === '/' || request.url === '') {
        response.statusCode = 302;
        response.setHeader('Location', '/portfolio/edit');
        response.end();
        return;
      }
...
export default {
  ...baseConfig,
  plugins: [...baseConfig.plugins, portfolioEditorAtRootPlugin()],
  server: {
    ...baseConfig.server,
